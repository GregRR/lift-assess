...
Milestone-17 result redesign. The table therefore summarizes only durable mapping facts that remain
meaningful under the current result model; obsolete aggregate alpha labels are intentionally omitted.

| Assembly pair | Source locus | Mapping outcome | Mapping count | All-chain size | Real time | User time | Max RSS |
|---|---|---|---:|---:|---:|---:|---:|
| `canFam3` → `canFam4` | `chrUn_JH373233:1845736-1845835` | multiple mappings | 170 | 2.47 GiB | 641.17 s | 639.14 s | 47.8 MiB |
| `canFam3` → `canFam4` | `chrUn_JH373233:1845736-1845835` | multiple mappings | 170 | 2.47 GiB | 625.71 s | 624.12 s | 46.4 MiB |
| `canFam3` → `canFam4` | `chr1:10000001-10000100` | one complete mapping | 1 | 2.47 GiB | 640.74 s | 638.09 s | 51.6 MiB |
| `canFam6` → `mm39` | `chr1:10000001-10000100` | no mapping | 0 | 157.0 MiB | 42.50 s | 42.14 s | 170.1 MiB |
| `canFam6` → `mm39` | `chr6:43585033-43585132` | one complete mapping | 1 | 157.0 MiB | 103.12 s | 102.39 s | 178.4 MiB |
...
resource. Comparing the simple `canFam3` → `canFam4` locus with the zero-mapping
...
The two `canFam6` → `mm39` runs also show that resource size is not the whole story. Once a mapping existed and comparative evidence was consumed, wall time rose from 42.50 to 103.12
seconds on the same assembly pair. Mapping-dependent comparative processing can therefore be material even when the chain
...
1. **Whole-chain traversal is a major single-locus cost.** A 170-mapping locus and a
   single-complete-mapping locus on the same large assembly pair took essentially the same time.
2. **Chain-resource size materially affects runtime.** Moving from the 2.47 GiB
   ...
   zero-mapping/simple-scan workload by roughly an order of magnitude.
3. **Comparative evidence still adds mapping-dependent work.** On the smaller assembly pair,
   the mapped locus took materially longer than the zero-mapping locus.
...
3. profile mapping-dependent comparative evidence lookup after the structural improvements;
...
validation. Every indexed/selective result below was required to reproduce the exact mapping
tuple from the full parser for both the 170-mapping mechanical fixture and the ordinary
single-mapping control.

| Access path | 170-mapping locus | 1-mapping locus | Notes |
|---|---:|---:|---|
...
| Full-resource blocked/bin index | 1.353 s | 0.106 s | 948 and 1 chain records selected; exact mapping equivalence |
...
`canFam4` COMPARATIVE resource set took 1.232 seconds after `sudo purge` and 1.192 seconds on a warm
...
COMPARATIVE resource set after `sudo purge`. That measurement is hardware/storage-specific, not a general
...
the exact queried genomic bins, and validates selected compressed blocks; the other cached resource set
...
indexing had made mapping lookup cheap. Parsing the 10.03-MiB ordinary net produced 847,501
...
reverse, flanking-interval, comparative-expansion, and batch work must share or index this traversal
...
order for reproducibility, and keep the existing parser/mapping logic authoritative for selected
...
resource set and never turns an ordinary first query into an implicit multi-minute preprocessing job.
